...
export function HabitsScreen({ onStartFocus }: { onStartFocus?: (context: HabitFocusContext) => void }) {
  ...
  return <section className="habits-screen">
    <div className="habits-day-head"><div><h2>Today’s habits</h2><p>{today.completedCount} of {today.plannedCount} completed</p></div><div className="habit-day-progress" aria-label={`${today.completedCount} of ${today.plannedCount} habits completed`}><span style={{ width: `${today.plannedCount ? today.completedCount / today.plannedCount * 100 : 0}%` }} /></div></div>
    <div className="habits-day-layout">
      <div className="habit-now-column">
        {current ? <article className="habit-now" style={{ "--habit-color": current.habit.color } as React.CSSProperties}>
      ...
      <div className="habit-queue-column">
        ...
        <div className="habit-queue">{grouped.later.length ? grouped.later.map((occurrence) => <article key={occurrence.id}><span className="habit-queue-icon" style={{ color: occurrence.habit.color }}><HabitIcon name={occurrence.habit.icon} /></span><div><strong>{occurrence.habit.name}</strong><span>{statusCopy(occurrence)}</span></div></article>) : <p>Nothing else is waiting.</p>}</div>
        <div className="habit-queue-head completed"><h3>Finished</h3><span>{grouped.completed.length}</span></div>
        <div className="habit-queue habit-finished">{grouped.completed.map((occurrence) => <article key={occurrence.id}><span className="habit-queue-icon"><HabitIcon name={occurrence.status === "completed" ? "check" : occurrence.habit.icon} /></span><div><strong>{occurrence.habit.name}</strong><span>{statusCopy(occurrence)}</span></div></article>)}</div>
      </div>
    </div>
  </section>;
}
