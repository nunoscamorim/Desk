...
function Preview({ data, config, onChange, accentColor, fontFamily = "Arial", canvas }: { data: DashboardData; config: WidgetConfig[]; onChange: (config: WidgetConfig[]) => void; accentColor: string; fontFamily?: string; canvas: CanvasSize }) {
  ...
  useEffect(() => { const t = setTimeout(() => { const calendar = document.querySelector<HTMLElement>(".configured-widget.configured-calendar"); if (!calendar) return; const before = calendar.getBoundingClientRect(); calendar.dispatchEvent(new PointerEvent("pointerdown", { bubbles: true, clientX: 100, clientY: 100 })); window.dispatchEvent(new PointerEvent("pointermove", { clientX: 100, clientY: 800 })); window.dispatchEvent(new PointerEvent("pointerup", { clientX: 100, clientY: 800 })); setTimeout(() => { const after = calendar.getBoundingClientRect(); console.log("DRAGTEST", JSON.stringify({ beforeTop: before.top, beforeBottom: before.bottom, afterTop: after.top, afterBottom: after.bottom })); }, 100); }, 1200); return () => clearTimeout(t); }, []);
