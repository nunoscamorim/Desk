...
export default function AdminPage() {
  ...
  const [saved, setSaved] = useState<{ widgets: WidgetConfig[]; accentColor: string; fontFamily: string } | null>(null);
  const [saveState, setSaveState] = useState<"idle" | "saving" | "saved" | "error">("idle");
  const [serviceStatus, setServiceStatus] = useState<Record<string, { configured: boolean }> | null>(null);
  const [authenticated, setAuthenticated] = useState<boolean | null>(null);
  const dirty = saved !== null && JSON.stringify({ widgets: config, accentColor, fontFamily }) !== JSON.stringify(saved);
  ...
  useEffect(() => { void fetch("/api/config").then((response) => response.json()).then((serverConfig: { widgets?: WidgetConfig[]; accentColor?: string; fontFamily?: string }) => { const next = { widgets: serverConfig.widgets ?? defaultWidgetConfig, accentColor: serverConfig.accentColor ?? "#c9ff52", fontFamily: serverConfig.fontFamily ?? "Arial" }; queueMicrotask(() => { setConfig(next.widgets); setAccentColor(next.accentColor); setFontFamily(next.fontFamily); setSaved(next); }); }).catch(() => { const next = { widgets: readWidgetConfig(window.localStorage.getItem("desk-dashboard-widgets")), accentColor: window.localStorage.getItem("desk-dashboard-accent") || "#c9ff52", fontFamily: window.localStorage.getItem("desk-dashboard-font") || "Arial" }; queueMicrotask(() => { setConfig(next.widgets); setAccentColor(next.accentColor); setFontFamily(next.fontFamily); setSaved(next); }); }); void loadData().then(setData).catch(() => setData(null)); }, []);
  useEffect(() => { const panel = document.querySelector(".widget-settings"); if (!panel || panel.querySelector(".font-setting")) return; const label = document.createElement("label"); label.className = "font-setting"; label.textContent = "Font"; const select = document.createElement("select"); ["Arial", "Inter", "Roboto", "Archivo", "DM Sans", "Space Grotesk", "Manrope"].forEach((font) => { const option = new Option(font, font); select.add(option); }); select.value = fontFamily; select.onchange = () => setFontFamily(select.value); label.appendChild(select); panel.appendChild(label); return () => label.remove(); }, [fontFamily, authenticated]);
  ...
  const save = async () => {
    setSaveState("saving");
    try {
      const response = await fetch("/api/config", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ widgets: config, accentColor, fontFamily }) });
      if (!response.ok) throw new Error("Save failed");
      const persisted = await response.json() as { widgets?: WidgetConfig[]; accentColor?: string; fontFamily?: string };
      const next = { widgets: persisted.widgets ?? config, accentColor: persisted.accentColor ?? accentColor, fontFamily: persisted.fontFamily ?? fontFamily };
      setConfig(next.widgets); setAccentColor(next.accentColor); setFontFamily(next.fontFamily); setSaved(next);
  ...
  const revert = () => { if (!saved) return; setConfig(saved.widgets); setAccentColor(saved.accentColor); setFontFamily(saved.fontFamily); setSaveState("idle"); };
