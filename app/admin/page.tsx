...
export default function AdminPage() {
  ...
  useEffect(() => { queueMicrotask(() => setConfig(readWidgetConfig(window.localStorage.getItem("desk-dashboard-widgets")))); void fetch("/api/config").then((response) => response.json()).then((serverConfig: { widgets?: WidgetConfig[]; accentColor?: string; fontFamily?: string }) => { queueMicrotask(() => { if (serverConfig.widgets) setConfig(serverConfig.widgets); if (serverConfig.accentColor) setAccentColor(serverConfig.accentColor); const locallySelectedFont = window.localStorage.getItem("desk-dashboard-font"); setFontFamily(serverConfig.fontFamily && (serverConfig.fontFamily !== "Arial" || !locallySelectedFont) ? serverConfig.fontFamily : locallySelectedFont || "Arial"); setConfigLoaded(true); }); }).catch(() => setConfigLoaded(true)); void loadData().then(setData).catch(() => setData(null)); }, []);
  useEffect(() => { if (!configLoaded) return; window.localStorage.setItem("desk-dashboard-widgets", JSON.stringify(config)); window.localStorage.setItem("desk-dashboard-font", fontFamily); void fetch("/api/config", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ widgets: config, accentColor, fontFamily }) }); }, [config, accentColor, fontFamily, configLoaded]);
  ...
  useEffect(() => { const panel = document.querySelector(".admin-header"); if (!panel || panel.querySelector(".config-actions")) return; const actions = document.createElement("div"); actions.className = "config-actions"; const save = document.createElement("button"); save.type = "button"; save.textContent = "Save as default"; save.onclick = () => { window.localStorage.setItem("desk-dashboard-default", JSON.stringify({ widgets: config, accentColor, fontFamily })); save.textContent = "Default saved"; window.setTimeout(() => { save.textContent = "Save as default"; }, 1400); }; const revert = document.createElement("button"); revert.type = "button"; revert.className = "secondary"; revert.textContent = "Revert to default"; revert.onclick = () => { try { const saved = JSON.parse(window.localStorage.getItem("desk-dashboard-default") || "null") as { widgets?: WidgetConfig[]; accentColor?: string; fontFamily?: string } | null; setConfig(saved?.widgets ?? defaultWidgetConfig); setAccentColor(saved?.accentColor ?? "#c9ff52"); setFontFamily(saved?.fontFamily ?? "Arial"); } catch { setConfig(defaultWidgetConfig); setAccentColor("#c9ff52"); setFontFamily("Arial"); } }; actions.append(save, revert); panel.appendChild(actions); return () => actions.remove(); }, [config, accentColor, fontFamily, authenticated]);
