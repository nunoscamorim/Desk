...
import { bentoArea, canvasPresets, defaultCanvas, normalizeCanvas, rescaleWidgetGeometry, type CanvasSize, type WidgetConfig, type WidgetType } from "@/lib/dashboard/config";
import { createWidgetInstance, defaultWidgetConfig, definitionFor, readWidgetConfig, widgetRegistry, widgetTypes } from "@/lib/dashboard/widget-registry";
...
export default function AdminPage() {
  ...
  const addWidget = (type: WidgetType) => { const instance = createWidgetInstance(type, config); const next = reflowBento([...config, instance], instance.id, bentoArea(canvas)); setConfig(next); setSelected(instance.id); setSaveState("idle"); };
  const duplicateWidget = (id: string) => { const source = config.find((widget) => widget.id === id); if (!source) return; const instance = { ...createWidgetInstance(source.type, config), ...{ width: source.width, height: source.height, settings: { ...source.settings } } }; const next = reflowBento([...config, instance], instance.id, bentoArea(canvas)); setConfig(next); setSelected(instance.id); setSaveState("idle"); };
  const removeWidget = (id: string) => { const next = config.filter((widget) => widget.id !== id); setConfig(next); if (selected === id) setSelected(next[0]?.id ?? null); setSaveState("idle"); };
  ...
  return <main className="admin-shell"><aside className="admin-sidebar"><div className="admin-brand"><span className="brand-mark">D</span><div><strong>Desk dashboard</strong><span>Configuration</span></div></div><nav className="admin-screens" aria-label="Admin screens"><p className="admin-eyebrow">Screens</p><a className="admin-screen active" href="#dashboard"><span>⌂</span>Dashboard</a><a className="admin-screen" href="#display"><span>▣</span>Display</a></nav><div className="admin-integrations"><p className="admin-eyebrow">Integrations</p>{integration("googleCalendar", "Google Calendar")}{integration("spotify", "Spotify")}{integration("weather", "Weather")}{integration("coolify", "Coolify")}</div><Link className="admin-back" href="/">← Back to dashboard</Link></aside><section className="admin-content"><header className="admin-header"><div><p className="admin-eyebrow">Dashboard / Configuration</p><h1>Make the desk yours.</h1><p>Configure what appears on the display. Save to publish the layout to the iPad.</p></div><div className="config-actions"><span className={`save-status ${saveState === "error" ? "error" : dirty ? "dirty" : "synced"}`}><span />{saveState === "error" ? "Couldn’t save — try again" : dirty ? "Unsaved changes" : "Saved to display"}</span><button type="button" className="secondary" onClick={revert} disabled={!dirty || saveState === "saving"}>Revert</button><button type="button" onClick={save} disabled={!dirty || saveState === "saving"}>{saveState === "saving" ? "Saving…" : dirty ? "Save changes" : "Saved"}</button></div></header><div className="admin-workspace"><section className="admin-panel widgets-panel"><div className="panel-heading"><div><p className="admin-eyebrow">Widgets on the display</p><h2>Choose what stays in view</h2></div><span>{config.filter((widget) => widget.enabled).length}/{config.length} enabled</span></div><div className="widget-list">{config.map((widget) => { const definition = definitionFor(widget.type); return <button type="button" className={`widget-row ${selected === widget.id ? "selected" : ""}`} onClick={() => setSelected(widget.id)} key={widget.id}><span className={`widget-symbol symbol-${widget.type}`}>{definition.symbol}</span><span className="widget-copy"><strong>{String(widget.settings.label ?? definition.label)}</strong><small>{definition.description}</small></span><span className={`toggle ${widget.enabled ? "on" : ""}`} onClick={(event) => { event.stopPropagation(); toggle(widget.id); }} role="switch" aria-checked={widget.enabled} aria-label={`Enable ${definition.label}`}><span /></span></button>; })}</div><div className="widget-add"><label>Add widget<select value="" onChange={(event) => { if (event.target.value) addWidget(event.target.value as WidgetType); }}><option value="">Choose a widget…</option>{widgetTypes.map((type) => <option key={type} value={type}>{widgetRegistry[type].label}</option>)}</select></label></div><div className="widget-settings"><p className="admin-eyebrow">Widget settings</p><h3>{selectedDefinition?.label ?? "No widget selected"}</h3>{selectedConfig ? <>{settingsControls}<div className="widget-instance-actions"><button type="button" className="secondary" onClick={() => duplicateWidget(selectedConfig.id)}>Duplicate</button><button type="button" className="danger" onClick={() => removeWidget(selectedConfig.id)}>Remove</button></div></> : <p className="settings-note">Add a widget to start arranging the display.</p>}<label className="accent-setting">Accent color<input type="color" value={accentColor} onChange={(event) => setAccentColor(event.target.value)} /></label>{fontControl}{canvasControl}<p className="settings-note">Drag or resize widgets in the preview. Save to publish positions, sizes, and accent color to the live display.</p></div></section><section className="admin-panel preview-panel"><div className="panel-heading"><div><p className="admin-eyebrow">Display preview</p><h2>What the desk sees</h2></div><span className="hardware-chip">{activePreset?.label ?? `${canvas.width} × ${canvas.height}`}</span></div>{data ? <Preview data={data} config={config} onChange={setConfig} accentColor={accentColor} fontFamily={fontFamily} canvas={canvas} /> : <div className="preview-loading">Loading preview…</div>}</section><TvPlaylistsPanel /></div></section></main>;
}
