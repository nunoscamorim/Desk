...
import { HABIT_CATEGORIES, habitColor } from "@/lib/habits/categories";
...
const emptyDraft = (): HabitDraft => ({ name: "", description: "", icon: "check", category: "custom", enabled: true, order: 0, days: [1, 2, 3, 4, 5], time: "09:00", windowBeforeMinutes: 0, windowAfterMinutes: 60, estimatedDurationMinutes: 5, reminders: { enabled: true, upcomingMinutes: 10, overdueMinutes: 15, defaultSnoozeMinutes: 10 }, steps: [] });
const templates: Array<{ label: string; draft: Partial<HabitDraft> }> = [
  ...
  { label: "Stretch", draft: { name: "Stretch", icon: "stretch", category: "movement", days: [1, 2, 3, 4, 5], time: "07:30", estimatedDurationMinutes: 5, steps: [{ id: "stretch-step", type: "check", title: "Complete a short stretch" }] } },
  { label: "Focus block", draft: { name: "Focus block", icon: "focus", category: "focus", days: [1, 2, 3, 4, 5], time: "10:00", windowBeforeMinutes: 30, windowAfterMinutes: 120, estimatedDurationMinutes: 25, steps: [{ id: "focus-step", type: "focus", title: "Complete a focus session", targetMinutes: 25 }] } },
  { label: "Deep work", draft: { name: "Deep work", icon: "focus", category: "focus", days: [1, 2, 3, 4, 5], time: "14:00", windowAfterMinutes: 180, estimatedDurationMinutes: 45, steps: [{ id: "deep-work-step", type: "focus", title: "One focused task", targetMinutes: 45 }] } },
  { label: "Hydrate", draft: { name: "Hydrate", icon: "water", category: "self-care", days: [0, 1, 2, 3, 4, 5, 6], time: "11:00", windowAfterMinutes: 1440, estimatedDurationMinutes: 5, steps: [{ id: "water-step", type: "check", title: "Finish a full glass of water" }] } },
  { label: "Walk", draft: { name: "Afternoon walk", icon: "stretch", category: "movement", days: [0, 1, 2, 3, 4, 5, 6], time: "16:00", windowAfterMinutes: 180, estimatedDurationMinutes: 20, steps: [{ id: "walk-step", type: "check", title: "Go for a walk" }] } },
  { label: "Workout", draft: { name: "Workout", icon: "workout", category: "workout", days: [1, 3, 5], time: "17:30", windowBeforeMinutes: 60, windowAfterMinutes: 240, estimatedDurationMinutes: 45, steps: [{ id: "workout-warmup", type: "check", title: "Warm up" }, { id: "workout-main", type: "focus", title: "Main lift", targetMinutes: 30 }, { id: "workout-cool", type: "check", title: "Cooldown" }] } },
  { label: "Morning routine", draft: { name: "Morning routine", icon: "sun", category: "morning", days: [0, 1, 2, 3, 4, 5, 6], time: "07:00", windowAfterMinutes: 90, estimatedDurationMinutes: 15, steps: [{ id: "morning-step", type: "check", title: "Complete your morning routine" }] } },
  { label: "Evening reset", draft: { name: "Evening reset", icon: "moon", category: "evening", days: [0, 1, 2, 3, 4, 5, 6], time: "19:00", windowAfterMinutes: 120, estimatedDurationMinutes: 10, steps: [{ id: "review", type: "check", title: "Review tomorrow" }, { id: "clear", type: "check", title: "Clear the desk" }] } },
  { label: "Reading", draft: { name: "Read a few pages", icon: "check", category: "custom", days: [0, 1, 2, 3, 4, 5, 6], time: "21:00", windowAfterMinutes: 120, estimatedDurationMinutes: 15, steps: [{ id: "reading-step", type: "check", title: "Read at least ten pages" }] } },
];
  ...
  return <>
    ...
    <div className="habits-admin-workspace">
      <section className="admin-panel habits-library">
        ...
        {loading ? <div className="habit-admin-loading"><span className="skeleton" /><span className="skeleton" /><span className="skeleton" /></div> : habits.length ? <div className="habit-admin-list">{habits.map((habit) => <button type="button" className={`habit-admin-row ${draft.id === habit.id ? "selected" : ""}`} key={habit.id} onClick={() => edit(habit)}><span className="habit-admin-icon" style={{ color: habitColor(habit.category), background: `color-mix(in srgb, ${habitColor(habit.category)} 16%, #252522)` }}><HabitIcon name={habit.icon} /></span><span className="habit-admin-copy"><strong>{habit.name}</strong><small>{habit.time} · {habit.days.length === 7 ? "Every day" : `${habit.days.length} days`} · {habit.estimatedDurationMinutes} min</small></span><span className={`habit-state-dot ${habit.enabled ? "on" : ""}`} aria-label={habit.enabled ? "Active" : "Paused"} /></button>)}</div> : <div className="empty-panel"><strong>No habits yet</strong><span>Choose a starter below or create one from scratch.</span></div>}
      ...
      <section className="admin-panel habit-editor">
        <div className="habit-editor-head"><div className="habit-editor-preview" style={{ color: habitColor(draft.category), background: `color-mix(in srgb, ${habitColor(draft.category)} 16%, #252522)` }}><HabitIcon name={draft.icon} /></div><div><h2>{draft.id ? draft.name : "New habit"}</h2><p>{draft.enabled ? "Active in your daily schedule" : "Paused"}</p></div></div>
        <div className="habit-form-grid">
          ...
          <Field label="Category"><select value={draft.category} onChange={(event) => update("category", event.target.value as HabitCategory)}>{Object.entries(HABIT_CATEGORIES).map(([value, { label }]) => <option key={value} value={value}>{label}</option>)}</select></Field>
        ...
        <div className="habit-icon-picker" role="group" aria-label="Habit icon">{habitIconOptions.map((icon) => <button type="button" key={icon.id} className={draft.icon === icon.id ? "selected" : ""} aria-label={icon.label} title={icon.label} onClick={() => update("icon", icon.id)}><HabitIcon name={icon.id} /></button>)}</div>
