...
export async function POST(request: Request) {
  ...
  const read = async <T>(action: string) => { api.search = new URLSearchParams({ ...params, action }).toString(); const response = await fetchWithRetry(api, { cache: "no-store" }, { label: `tv:xtream:${action}` }); if (!response.ok) throw new Error(`Xtream request failed (${response.status})`); return response.json() as Promise<T>; };
  try {
    const [categories, streams] = await Promise.all([read<Array<{ category_id?: string; category_name?: string }>>("get_live_categories"), read<Array<{ stream_id?: number; name?: string; category_id?: string; stream_icon?: string }>>("get_live_streams")]);
