import type { SpotifyNowPlaying, SpotifyPlaylist, SpotifyRecentTrack } from "@/lib/dashboard/types";
export type SpotifyPlaybackAction = "play" | "pause" | "next" | "previous" | "seek" | "shuffle" | "repeat";
export type SpotifyPlaybackValue = number | boolean | "track" | "context" | "off";
export interface SpotifyService { getNowPlaying(): Promise<SpotifyNowPlaying | null>; getRecentlyPlayed(): Promise<SpotifyRecentTrack[]>; getPlaylists(): Promise<SpotifyPlaylist[]>; controlPlayback(action: SpotifyPlaybackAction, value?: SpotifyPlaybackValue, contextUri?: string): Promise<void>; }
...
export class MockSpotifyService implements SpotifyService {
  ...
  async getPlaylists(): Promise<SpotifyPlaylist[]> { return []; }
  async controlPlayback(): Promise<void> { throw new Error("Spotify is not connected"); }
}
