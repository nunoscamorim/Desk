...
type XtreamStream = { stream_id?: number; name?: string; stream_icon?: string; category_id?: string; tvg_id?: string; epg_channel_id?: string; container_extension?: string; stream_type?: string };
...
async function loadXtream(playlist: Extract<Playlist, { source: "xtream" }>): Promise<{ channels: Channel[]; truncated: boolean }> {
  ...
  return { channels: filtered.filter((stream) => Number.isFinite(stream.stream_id)).map((stream) => { const extension = stream.container_extension?.trim() || (stream.stream_type === "hls" ? "m3u8" : "ts"); return { id: String(stream.stream_id), tvgId: stream.epg_channel_id?.trim() || stream.tvg_id?.trim() || null, name: stream.name?.trim() || "Untitled channel", logo: stream.stream_icon || null, group: categoryNames.get(String(stream.category_id ?? "")) ?? "General", url: `${xtreamBase(playlist.server)}/live/${encodeURIComponent(playlist.username)}/${encodeURIComponent(playlist.password)}/${stream.stream_id}.${extension}` }; }), truncated: false };
}
